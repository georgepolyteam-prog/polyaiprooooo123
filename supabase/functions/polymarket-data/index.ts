...
function extractOutcomeNames(market: any): string {
  ...
  if (market.outcomes && Array.isArray(market.outcomes)) {
    outcomeText += market.outcomes.map((o: any) => (typeof o === "string" ? o : o.name || o.outcome || "")).join(" ");
  }
  ...
  try {
          ...
          const outcomes = markets
            ...
            .filter((o: any) => {
              ...
              if (o.yesPrice >= 0.99 || o.noPrice >= 0.99) {
                console.log(
                  `[GetEvents] Filtering effectively-resolved market (${Math.round(o.yesPrice * 100)}%): ${o.question}`,
                );
                return false;
              }
    ...
    if (action === "search") {
      ...
      return new Response(
        JSON.stringify({
          ...
          searchFailed: scoredMarkets.length === 0, // Flag for -chat to handle gracefully
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (action === "getMarketDetails") {
      ...
      if (!market) {
        ...
        const eventsResponse = await fetch(`https://gamma-api.polymarket.com/events?slug=${marketSlug}`, {
          headers: getApiHeaders(),
        });
    ...
    if (action === "getStats") {
      ...
      const response = await fetch(`https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=500`, {
        headers: getApiHeaders(),
      });
